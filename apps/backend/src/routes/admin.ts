...
router.get('/admin/groups/:name', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!group) { res.status(404).json({ error: { message: 'Groupe introuvable' } }); return }
    res.json({ group })
  } catch (err) { next(err) }
})
...
router.post('/admin/groups/:name/members', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { email } = req.body
    if (!email) { res.status(400).json({ error: { message: 'Email requis' } }); return }
    const group = await addMember(String(req.params.name), email)
    res.json({ group })
  } catch (err) { next(err) }
})

router.delete('/admin/groups/:name/members/:email', async (req: Request, res: Response, next: NextFunction) => {
  try {
